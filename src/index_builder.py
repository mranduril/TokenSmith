...
def build_index(
    ...
    *,
    ...
    if experimental_chunking:
        print("Using experimental chunking strategy.")
        assert docling_json is not None, "docling_json path must be provided for experimental chunking"
        all_chunks, sources, metadata, page_to_chunk_ids = chunk_docling_hierarchical_json(docling_json)
    
    else:
        # Fall back to standard chunking strategy for non-experimental runs
